...
import static com.avos.avoscloud.okio.Util.UTF_8;
...
public final class BufferTest {
  ...
  @Test public void completeSegmentByteCountOnBufferWithFullSegments() throws Exception {
    Buffer buffer = new Buffer();
    buffer.writeUtf8(TestUtil.repeat('a', Segment.SIZE * 4));
  ...
  @Test public void completeSegmentByteCountOnBufferWithIncompleteTailSegment() throws Exception {
    Buffer buffer = new Buffer();
    buffer.writeUtf8(TestUtil.repeat('a', Segment.SIZE * 4 - 10));
  ...
  @Test public void multipleSegmentBuffers() throws Exception {
    ...
    assertEquals(TestUtil.repeat('a', 999), buffer.readUtf8(999)); // a...a
    assertEquals("a" + TestUtil.repeat('b', 2500) + "c", buffer.readUtf8(2502)); // ab...bc
    assertEquals(TestUtil.repeat('c', 4998), buffer.readUtf8(4998)); // c...c
    assertEquals("c" + TestUtil.repeat('d', 10000) + "e", buffer.readUtf8(10002)); // cd...de
    assertEquals(TestUtil.repeat('e', 24998), buffer.readUtf8(24998)); // e...e
    assertEquals("e" + TestUtil.repeat('f', 50000), buffer.readUtf8(50001)); // ef...f
    assertEquals(0, buffer.size());
  }
  ...
  @Test public void writeSplitSourceBufferLeft() throws Exception {
    ...
    sink.writeUtf8(TestUtil.repeat('b', Segment.SIZE - 10));

    Buffer source = new Buffer();
    source.writeUtf8(TestUtil.repeat('a', Segment.SIZE * 2));
  ...
  @Test public void writePrefixDoesntSplitButRequiresCompact() throws Exception {
    Buffer sink = new Buffer();
    sink.writeUtf8(TestUtil.repeat('b', Segment.SIZE - 10)); // limit = size - 10
  ...
  @Test public void copyToSpanningSegments() throws Exception {
    Buffer source = new Buffer();
    source.writeUtf8(TestUtil.repeat('a', Segment.SIZE * 2));
    source.writeUtf8(TestUtil.repeat('b', Segment.SIZE * 2));

    ByteArrayOutputStream out = new ByteArrayOutputStream();
    source.copyTo(out, 10, Segment.SIZE * 3);

    Assert.assertEquals(TestUtil.repeat('a', Segment.SIZE * 2 - 10) + TestUtil.repeat('b', Segment.SIZE + 10),
            out.toString());
    assertEquals(TestUtil.repeat('a', Segment.SIZE * 2) + TestUtil.repeat('b', Segment.SIZE * 2),
  ...
  @Test public void writeToSpanningSegments() throws Exception {
    Buffer buffer = new Buffer();
    buffer.writeUtf8(TestUtil.repeat('a', Segment.SIZE * 2));
    buffer.writeUtf8(TestUtil.repeat('b', Segment.SIZE * 2));
    ...
    Assert.assertEquals(TestUtil.repeat('a', Segment.SIZE * 2 - 10) + TestUtil.repeat('b', Segment.SIZE + 10),
            out.toString());
    assertEquals(TestUtil.repeat('b', Segment.SIZE - 10), buffer.readUtf8(buffer.size));
  }
  ...
  @Test public void readFromSpanningSegments() throws Exception {
    InputStream in = new ByteArrayInputStream("hello, world!".getBytes(UTF_8));
    Buffer buffer = new Buffer().writeUtf8(TestUtil.repeat('a', Segment.SIZE - 10));
    buffer.readFrom(in);
    String out = buffer.readUtf8();
    assertEquals(TestUtil.repeat('a', Segment.SIZE - 10) + "hello, world!", out);
  }
  ...
  @Test public void moveAllRequestedBytesWithRead() throws Exception {
    ...
    assertEquals(TestUtil.repeat('a', 10) + TestUtil.repeat('b', 10), sink.readUtf8(20));
  }
  ...
  @Test public void moveFewerThanRequestedBytesWithRead() throws Exception {
    ...
    assertEquals(TestUtil.repeat('a', 10) + TestUtil.repeat('b', 20), sink.readUtf8(30));
  }
  ...
  @Test public void byteAt() throws Exception {
    Buffer buffer = new Buffer();
    buffer.writeUtf8("a");
    buffer.writeUtf8(TestUtil.repeat('b', Segment.SIZE));
    buffer.writeUtf8("c");
  ...
  @Test public void cloneMultipleSegments() throws Exception {
    Buffer original = new Buffer();
    original.writeUtf8(TestUtil.repeat('a', Segment.SIZE * 3));
    Buffer clone = original.clone();
    original.writeUtf8(TestUtil.repeat('b', Segment.SIZE * 3));
    clone.writeUtf8(TestUtil.repeat('c', Segment.SIZE * 3));

    assertEquals(TestUtil.repeat('a', Segment.SIZE * 3) + TestUtil.repeat('b', Segment.SIZE * 3),
        original.readUtf8(Segment.SIZE * 6));
    assertEquals(TestUtil.repeat('a', Segment.SIZE * 3) + TestUtil.repeat('c', Segment.SIZE * 3),
  ...
  @Test public void copyToOnSegmentBoundary() throws Exception {
    String as = TestUtil.repeat('a', Segment.SIZE);
    String bs = TestUtil.repeat('b', Segment.SIZE);
    String cs = TestUtil.repeat('c', Segment.SIZE);
    String ds = TestUtil.repeat('d', Segment.SIZE);
  ...
  @Test public void copyToOffSegmentBoundary() throws Exception {
    String as = TestUtil.repeat('a', Segment.SIZE - 1);
    String bs = TestUtil.repeat('b', Segment.SIZE + 2);
    String cs = TestUtil.repeat('c', Segment.SIZE - 4);
    String ds = TestUtil.repeat('d', Segment.SIZE + 8);
