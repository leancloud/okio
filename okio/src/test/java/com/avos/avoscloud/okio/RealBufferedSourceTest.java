...
public final class RealBufferedSourceTest {
  @Test public void inputStreamTracksSegments() throws Exception {
    ...
    source.writeUtf8("a");
    source.writeUtf8(TestUtil.repeat('b', Segment.SIZE));
    source.writeUtf8("c");
    ...
    Assert.assertEquals(TestUtil.repeat('b', Segment.SIZE - 1), new String(data, 0, Segment.SIZE - 1, UTF_8));
    assertEquals(2, source.size());
  ...
  @Test public void requireReadsOneSegmentAtATime() throws Exception {
    Buffer source = new Buffer();
    source.writeUtf8(TestUtil.repeat('a', Segment.SIZE));
    source.writeUtf8(TestUtil.repeat('b', Segment.SIZE));
