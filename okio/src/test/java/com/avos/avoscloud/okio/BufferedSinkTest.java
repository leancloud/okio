...
public class BufferedSinkTest {
  ...
  @Test public void writeLastByteInSegment() throws Exception {
    ...
    assertEquals(TestUtil.repeat('a', Segment.SIZE - 1), data.readUtf8(Segment.SIZE - 1));
  ...
  @Test public void writeLastIntegerInSegment() throws Exception {
    ...
    assertEquals(TestUtil.repeat('a', Segment.SIZE - 4), data.readUtf8(Segment.SIZE - 4));
    assertEquals("Buffer[size=8 data=abcdef0187654321]", data.toString());
  }
  ...
  @Test public void writeIntegerDoesNotQuiteFitInSegment() throws Exception {
    ...
    assertEquals(TestUtil.repeat('a', Segment.SIZE - 3), data.readUtf8(Segment.SIZE - 3));
    assertEquals("Buffer[size=8 data=abcdef0187654321]", data.toString());
  }
  ...
  @Test public void outputStream() throws Exception {
    ...
    out.write('a');
    out.write(TestUtil.repeat('b', 9998).getBytes(UTF_8));
    out.write('c');
    out.flush();
    assertEquals("a" + TestUtil.repeat('b', 9998) + "c", data.readUtf8());
  }
