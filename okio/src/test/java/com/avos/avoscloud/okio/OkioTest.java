...
public final class OkioTest {
  ...
  @Test public void sourceFromInputStream() throws Exception {
    InputStream in = new ByteArrayInputStream(
        ("a" + TestUtil.repeat('b', Segment.SIZE * 2) + "c").getBytes(UTF_8));
    ...
    assertEquals(TestUtil.repeat('b', Segment.SIZE), sink.readUtf8());

    // Source: b...bc. Sink: b...bc.
    assertEquals(Segment.SIZE - 1, source.read(sink, 20000));
    assertEquals(TestUtil.repeat('b', Segment.SIZE - 2) + "c", sink.readUtf8());
