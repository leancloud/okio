...
public final class DeflaterSinkTest {
  ...
  @Test public void deflateWellCompressed() throws IOException {
    String original = TestUtil.repeat('a', 1024 * 1024);
    Buffer data = new Buffer();
    data.writeUtf8(original);
    Buffer sink = new Buffer();
    DeflaterSink deflaterSink = new DeflaterSink(sink, new Deflater());
  ...
  @Test public void deflatePoorlyCompressed() throws IOException {
    ByteString original = TestUtil.randomBytes(1024 * 1024);
    Buffer data = new Buffer();
    data.write(original);
    Buffer sink = new Buffer();
    DeflaterSink deflaterSink = new DeflaterSink(sink, new Deflater());
  ...
  @Test public void multipleSegmentsWithoutCompression() throws IOException {
    ...
    deflaterSink.write(new Buffer().writeUtf8(TestUtil.repeat('a', byteCount)), byteCount);
    deflaterSink.close();
    assertEquals(TestUtil.repeat('a', byteCount), inflate(buffer).readUtf8(byteCount));
  }
  ...
  @Test public void deflateIntoNonemptySink() throws Exception {
    ...
    for (int i = 0; i < Segment.SIZE; i++) {
      ...
      Buffer sink = new Buffer().writeUtf8(TestUtil.repeat('a', i));
  ...
  @Test public void closeWithExceptionWhenWritingAndClosing() throws IOException {
    ...
    deflaterSink.write(new Buffer().writeUtf8(TestUtil.repeat('a', Segment.SIZE)), Segment.SIZE);
    try {
      deflaterSink.close();
      fail();
